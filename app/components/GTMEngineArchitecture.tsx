...
function StickyCard({ step, index, total }: { step: any, index: number, total: number }) {
    ...
    return (
        ...
        >
            <motion.div 
                style={{ scale, opacity }}
                className="w-full max-w-7xl h-[80vh] md:h-[70vh] bg-[#0A0A0A] border border-white/10 rounded-lg overflow-hidden flex flex-col md:grid md:grid-cols-2 shadow-[0_-20px_100px_rgba(0,0,0,0.5)]"
            >
                {/* --- LEFT SIDE: CONTENT --- */}
                <div className="p-10 md:p-16 flex flex-col justify-center border-b md:border-b-0 md:border-r border-white/5 relative">
                    <div className={`absolute top-0 left-0 w-full h-full ${step.bgGlow} blur-[100px] pointer-events-none`} />
                    
                    <div className="relative z-10">
                        <div className={`text-[10px] font-mono ${step.color} uppercase tracking-[0.3em] mb-6`}>
                            PHASE 0{index + 1}
                        </div>
                        <h3 className="text-display-3 text-text-primary mb-6 leading-tight">
                            {step.title}
                        </h3>
                        <p className="text-body text-text-secondary mb-10 leading-relaxed max-w-md">
                            {step.desc}
                        </p>
                        ...
                        <div className="space-y-4">
                            {step.items.map((item: string, idx: number) => (
                                <div key={idx} className="flex items-center gap-3">
                                    ...
                                    <span className="text-ui-xs text-text-tertiary">{item}</span>
                                </div>
                            ))}
                        </div>
                    </div>
                ...
                <div className="bg-[#050505] p-6 md:p-10 flex items-center justify-center relative overflow-hidden">
