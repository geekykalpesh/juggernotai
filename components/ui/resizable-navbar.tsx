    ...
    return (
        <motion.div
            ...
            className={cn(
                ...
                visible && "bg-brand-surface/80 backdrop-blur-md border border-brand-border/50 shadow-2xl",
                className,
            )}
        >
            {children}
        ...
        >
            {items.map((item, idx) => (
                <a
                    ...
                    onClick={onItemClick}
                    className="relative px-4 py-2 text-text-secondary hover:text-white transition-colors"
                    key={`link-${idx}`}
                    href={item.link}
                >
                    {hovered === idx && (
    ...
    return (
        <motion.div
            ...
            className={cn(
                ...
                visible && "bg-brand-surface/80 backdrop-blur-md border border-brand-border/50",
                className,
            )}
        >
            {children}
...
export const NavbarLogo = ({ visible }: { visible?: boolean }) => {
    return (
        ...
        >
            <span className="text-xl font-space-grotesk font-bold tracking-tight text-white uppercase">
                JUGGERNOT<span className="text-white/50">AI</span>
            </span>
        </a>
    );
};
